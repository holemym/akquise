...
- [x] T5-6 Einsatz 2 an Zeraq übergeben (2026-09-16): **Kartbahn Wien** (der fertige Pitch aus `../kartbahn`) + **Notariat Lukanec** (eine Entscheiderin, gemessener Aufhänger). `data/EINSATZ-2.md` = deutscher Einsatzleitfaden (Material, Zeitplan, Besuchsskript, Angebotsstufen, Einwände, Nicht-sagen, Rückmeldung); `tools/einsatz.py` = zwei handgeschriebene Briefe mit QR → jeweils eigener noindex-Link; `tools/deck_pdf.py` = Druckunterlagen zum Dalassen. Lukanec-Deck LIVE unter eigener noindex-URL lukanec-befund.vercel.app (der Dashboard-Pfad geht nie an Kunden). Dashboard hat eine Seite *Einsatz 2*. **Vor dem Versand: David trägt die Absenderadresse in `config.json` ein und startet `tools/einsatz.py` neu.**
...
## Session-Log
- 2026-09-16 — Einsatz 2 für Zeraq vorbereitet: Kartbahn Wien + Notariat Lukanec (T5-6). Befunde am selben Tag live nachgeprüft (Kartbahn /gutschein + /kindergeburtstage weiter 404, Rosental-Text und „ab 10. April“ weiter online; Lukanec weiter ohne Meta-Beschreibung, ohne H1, © 2020). Nebenbei behoben: die Decks zeigten „Lighthouse“ auf Kundenfolien (`esc()` entfernt jetzt auch Werkzeugnamen), und die Dokumente-Links im Dashboard waren noch relativ (der 404-Fehler vom 08.09.). Heredoc-Falle wieder zugeschlagen (`` wurde zum Steuerzeichen) — per Byte-Ersetzung behoben.
